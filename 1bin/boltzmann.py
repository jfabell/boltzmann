...
class BoltzmannDSMC:
    def __init__(self, num_steps, name, restart_cyclenum, Nc, gridpts, bnf, num_nodes, pyk, resample_freq, in_dt, in_spc):
        ...
        self.target_ppc_LE_ar = int(self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
    ...
    def _check_for_resampling(self):
        ...
        self.bin_particles(self.big_tosum_ar_list[ng], self.d_cell_particles_LE_ar, 
                           self.d_bin_write_inds_LE_ar, self.d_currxbins_list[ng], 
                           self.Nc_list[ng], self.N, self.max_ppc_realistic_LE, 
                           self.threads_per_block)
