...
class BoltzmannDSMC:
    def __init__(self, num_steps, name, restart_cyclenum, Nc, gridpts, bnf, num_nodes, pyk, resample_freq, in_dt, in_spc):
        ...
        self.target_ppc_LE_ar = int(0.4*self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
        self.target_ppc_ME1_ar = int(0.3*self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
        self.target_ppc_ME2_ar = int(0.2*self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
        self.target_ppc_ME3_ar = int(0.07*self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
        self.target_ppc_HE_ar = int(0.03*self.Nc_list[0]/self.N) * cp.ones(self.N+1).astype(int)
        self.ttLE = self.target_ppc_LE_ar[1]
        self.ttME1 = self.target_ppc_ME1_ar[1]
        self.ttME2 = self.target_ppc_ME2_ar[1]
        self.ttME3 = self.target_ppc_ME3_ar[1]
        ...
        # Initializing to condition from Liu
            ...
            self.Nc_list[ng] = int(np.sum(self.target_ppc_LE_ar) + \
                                   np.sum(self.target_ppc_HE_ar) + \
                                   np.sum(self.target_ppc_ME1_ar) + \
                                   np.sum(self.target_ppc_ME2_ar) + \
                                   np.sum(self.target_ppc_ME3_ar))
            ...
            for ii in range(self.N+1):
                ## FOR FIXED PPC INIT
                num_cell = int(self.target_ppc_LE_ar[ii] + \
                               self.target_ppc_ME1_ar[ii] + \
                               self.target_ppc_ME2_ar[ii] + \
                               self.target_ppc_ME3_ar[ii] + \
                               self.target_ppc_HE_ar[ii])
                # put in the x's
        ...
        self.bin_write_inds_ME1_ar = cp.zeros(self.N+1).astype(int)
        self.d_bin_write_inds_ME1_ar = cuda.to_device(self.bin_write_inds_ME1_ar)

        self.bin_write_inds_ME2_ar = cp.zeros(self.N+1).astype(int)
        self.d_bin_write_inds_ME2_ar = cuda.to_device(self.bin_write_inds_ME2_ar)

        self.bin_write_inds_ME3_ar = cp.zeros(self.N+1).astype(int)
        self.d_bin_write_inds_ME3_ar = cuda.to_device(self.bin_write_inds_ME3_ar)
        ...
        self.max_ppc_realistic_ME1 = int(10.0*np.max(self.target_ppc_ME1_ar))
        self.max_ppc_realistic_ME2 = int(10.0*np.max(self.target_ppc_ME2_ar))
        self.max_ppc_realistic_ME3 = int(10.0*np.max(self.target_ppc_ME3_ar))
        ...
        self.cell_particles_ME1_ar = cp.zeros((self.max_ppc_realistic_ME1,self.N+1)).astype(int)
        self.d_cell_particles_ME1_ar = cuda.to_device(self.cell_particles_ME1_ar)

        self.cell_particles_ME2_ar = cp.zeros((self.max_ppc_realistic_ME2,self.N+1)).astype(int)
        self.d_cell_particles_ME2_ar = cuda.to_device(self.cell_particles_ME2_ar)

        self.cell_particles_ME3_ar = cp.zeros((self.max_ppc_realistic_ME3,self.N+1)).astype(int)
        self.d_cell_particles_ME3_ar = cuda.to_device(self.cell_particles_ME3_ar)
    ...
    def _do_resampling(self):
        ...
        for cc in range(0, self.N+1):
            ...
            tppc_ME1 = int(self.target_ppc_ME1_ar[cc])
            tppc_ME2 = int(self.target_ppc_ME2_ar[cc])
            tppc_ME3 = int(self.target_ppc_ME3_ar[cc])
            tppc_HE = int(self.target_ppc_HE_ar[cc])
            curr_ppc_LE = int(self.bin_write_inds_LE_ar[cc])
            curr_ppc_ME1 = int(self.bin_write_inds_ME1_ar[cc])
            curr_ppc_ME2 = int(self.bin_write_inds_ME2_ar[cc])
            curr_ppc_ME3 = int(self.bin_write_inds_ME3_ar[cc])
            ...
            ## Too few medium1 energies
            if (0 < curr_ppc_ME1 < 0.5*self.target_ppc_ME1_ar[cc]):
                if (curr_ppc_ME1 == 0):
                    continue
                ids_ME1 = self.cell_particles_ME1_ar[0:curr_ppc_ME1,cc]
                HW_ids_temp = cp.where(self.big_tosum_ar_list[ng][ids_ME1,self.wt] > self.min_wt_allow)[0]
                HW_ids = ids_ME1[HW_ids_temp]

                if (len(HW_ids) > 0):
                    P_wts = self.big_tosum_ar_list[ng][HW_ids,self.wt]/cp.sum(self.big_tosum_ar_list[ng][HW_ids,self.wt])
                    keepers = cp.random.choice(HW_ids,2*tppc_ME1-curr_ppc_ME1,True,p=P_wts)
                    unique_ids, unique_cts = cp.unique(keepers,return_counts=True)
                    n_unique = len(unique_ids)
            ...
            ## Too many medium1 energies
            if (curr_ppc_ME1 >= 3.0*self.target_ppc_ME1_ar[cc]):
                old_Nc = self.Nc_list[ng]
                curr_ppc_ME1 = min(curr_ppc_ME1, self.max_ppc_realistic_ME1)
                temp_tppc_ME1 = int(0.6*self.target_ppc_ME1_ar[cc])

                ids_ME1 = self.cell_particles_ME1_ar[0:curr_ppc_ME1,cc]
                original_wt = cp.sum(self.big_tosum_ar_list[ng][ids_ME1,self.wt])

                # v's and Te before
                v_cell_x = cp.sum(self.big_data_ar_list[ng][ids_ME1,self.vx]*self.big_tosum_ar_list[ng][ids_ME1,self.wt])/original_wt
                v_cell_y = cp.sum(self.big_data_ar_list[ng][ids_ME1,self.vy]*self.big_tosum_ar_list[ng][ids_ME1,self.wt])/original_wt
                v_cell_z = cp.sum(self.big_data_ar_list[ng][ids_ME1,self.vz]*self.big_tosum_ar_list[ng][ids_ME1,self.wt])/original_wt
                T_cell = (2/3) * cp.sum(self.big_tosum_ar_list[ng][ids_ME1,self.wt]*self.big_tosum_ar_list[ng][ids_ME1,self.en] / (original_wt))

                # new wt for particles
                wbar = original_wt / temp_tppc_ME1

                # use strata sampling for partial sum array 
                # to select kept particles
                p_use = self.big_tosum_ar_list[ng][ids_ME1,self.wt] / original_wt
                w_cumsum = cp.cumsum(p_use)
                Rvals = (1/temp_tppc_ME1) * (cp.random.rand(temp_tppc_ME1) + cp.arange(temp_tppc_ME1))
                keep_ids = cp.searchsorted(w_cumsum,Rvals)

                num_keepids = len(keep_ids)
                self.big_data_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_data_ar_list[ng][ids_ME1[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_tosum_ar_list[ng][ids_ME1[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,self.wt] = wbar*cp.ones(num_keepids)

                self.big_data_ar_list[ng][ids_ME1,:] = cp.zeros((curr_ppc_ME1,6))
                self.big_tosum_ar_list[ng][ids_ME1,:] = cp.zeros((curr_ppc_ME1,4))
            ...
            ## Too few medium2 energies
            if (0 < curr_ppc_ME2 < 0.5*self.target_ppc_ME2_ar[cc]):
                if (curr_ppc_ME2 == 0):
                    continue
                ids_ME2 = self.cell_particles_ME2_ar[0:curr_ppc_ME2,cc]
                HW_ids_temp = cp.where(self.big_tosum_ar_list[ng][ids_ME2,self.wt] > self.min_wt_allow)[0]
                HW_ids = ids_ME2[HW_ids_temp]

                if (len(HW_ids) > 0):
                    P_wts = self.big_tosum_ar_list[ng][HW_ids,self.wt]/cp.sum(self.big_tosum_ar_list[ng][HW_ids,self.wt])
                    keepers = cp.random.choice(HW_ids,2*tppc_ME2-curr_ppc_ME2,True,p=P_wts)
                    unique_ids, unique_cts = cp.unique(keepers,return_counts=True)
                    n_unique = len(unique_ids)
            ...
            ## Too many medium2 energies
            if (curr_ppc_ME2 >= 3.0*self.target_ppc_ME2_ar[cc]):
                old_Nc = self.Nc_list[ng]
                curr_ppc_ME2 = min(curr_ppc_ME2, self.max_ppc_realistic_ME2)
                temp_tppc_ME2 = int(0.6*self.target_ppc_ME2_ar[cc])

                ids_ME2 = self.cell_particles_ME2_ar[0:curr_ppc_ME2,cc]
                original_wt = cp.sum(self.big_tosum_ar_list[ng][ids_ME2,self.wt])

                # v's and Te before
                v_cell_x = cp.sum(self.big_data_ar_list[ng][ids_ME2,self.vx]*self.big_tosum_ar_list[ng][ids_ME2,self.wt])/original_wt
                v_cell_y = cp.sum(self.big_data_ar_list[ng][ids_ME2,self.vy]*self.big_tosum_ar_list[ng][ids_ME2,self.wt])/original_wt
                v_cell_z = cp.sum(self.big_data_ar_list[ng][ids_ME2,self.vz]*self.big_tosum_ar_list[ng][ids_ME2,self.wt])/original_wt
                T_cell = (2/3) * cp.sum(self.big_tosum_ar_list[ng][ids_ME2,self.wt]*self.big_tosum_ar_list[ng][ids_ME2,self.en] / (original_wt))

                # new wt for particles
                wbar = original_wt / temp_tppc_ME2

                # use strata sampling for partial sum array 
                # to select kept particles
                p_use = self.big_tosum_ar_list[ng][ids_ME2,self.wt] / original_wt
                w_cumsum = cp.cumsum(p_use)
                Rvals = (1/temp_tppc_ME2) * (cp.random.rand(temp_tppc_ME2) + cp.arange(temp_tppc_ME2))
                keep_ids = cp.searchsorted(w_cumsum,Rvals)

                num_keepids = len(keep_ids)
                self.big_data_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_data_ar_list[ng][ids_ME2[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_tosum_ar_list[ng][ids_ME2[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,self.wt] = wbar*cp.ones(num_keepids)

                self.big_data_ar_list[ng][ids_ME2,:] = cp.zeros((curr_ppc_ME2,6))
                self.big_tosum_ar_list[ng][ids_ME2,:] = cp.zeros((curr_ppc_ME2,4))
            ...
            ## Too few medium3 energies
            if (0 < curr_ppc_ME3 < 0.5*self.target_ppc_ME3_ar[cc]):
                if (curr_ppc_ME3 == 0):
                    continue
                ids_ME3 = self.cell_particles_ME3_ar[0:curr_ppc_ME3,cc]
                HW_ids_temp = cp.where(self.big_tosum_ar_list[ng][ids_ME3,self.wt] > self.min_wt_allow)[0]
                HW_ids = ids_ME3[HW_ids_temp]

                if (len(HW_ids) > 0):
                    P_wts = self.big_tosum_ar_list[ng][HW_ids,self.wt]/cp.sum(self.big_tosum_ar_list[ng][HW_ids,self.wt])
                    keepers = cp.random.choice(HW_ids,2*tppc_ME3-curr_ppc_ME3,True,p=P_wts)
                    unique_ids, unique_cts = cp.unique(keepers,return_counts=True)
                    n_unique = len(unique_ids)
            ...
            ## Too many medium3 energies
            if (curr_ppc_ME3 >= 3.0*self.target_ppc_ME3_ar[cc]):
                old_Nc = self.Nc_list[ng]
                curr_ppc_ME3 = min(curr_ppc_ME3, self.max_ppc_realistic_ME3)
                temp_tppc_ME3 = int(0.6*self.target_ppc_ME3_ar[cc])

                ids_ME3 = self.cell_particles_ME3_ar[0:curr_ppc_ME3,cc]
                original_wt = cp.sum(self.big_tosum_ar_list[ng][ids_ME3,self.wt])

                # v's and Te before
                v_cell_x = cp.sum(self.big_data_ar_list[ng][ids_ME3,self.vx]*self.big_tosum_ar_list[ng][ids_ME3,self.wt])/original_wt
                v_cell_y = cp.sum(self.big_data_ar_list[ng][ids_ME3,self.vy]*self.big_tosum_ar_list[ng][ids_ME3,self.wt])/original_wt
                v_cell_z = cp.sum(self.big_data_ar_list[ng][ids_ME3,self.vz]*self.big_tosum_ar_list[ng][ids_ME3,self.wt])/original_wt
                T_cell = (2/3) * cp.sum(self.big_tosum_ar_list[ng][ids_ME3,self.wt]*self.big_tosum_ar_list[ng][ids_ME3,self.en] / (original_wt))

                # new wt for particles
                wbar = original_wt / temp_tppc_ME3

                # use strata sampling for partial sum array 
                # to select kept particles
                p_use = self.big_tosum_ar_list[ng][ids_ME3,self.wt] / original_wt
                w_cumsum = cp.cumsum(p_use)
                Rvals = (1/temp_tppc_ME3) * (cp.random.rand(temp_tppc_ME3) + cp.arange(temp_tppc_ME3))
                keep_ids = cp.searchsorted(w_cumsum,Rvals)

                num_keepids = len(keep_ids)
                self.big_data_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_data_ar_list[ng][ids_ME3[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,:] = self.big_tosum_ar_list[ng][ids_ME3[keep_ids],:]
                self.big_tosum_ar_list[ng][self.Nc_list[ng]:self.Nc_list[ng]+num_keepids,self.wt] = wbar*cp.ones(num_keepids)

                self.big_data_ar_list[ng][ids_ME3,:] = cp.zeros((curr_ppc_ME3,6))
                self.big_tosum_ar_list[ng][ids_ME3,:] = cp.zeros((curr_ppc_ME3,4))
    ...
    def _check_for_resampling(self):
        ...
        self.bin_write_inds_ME1_ar.fill(0)
        self.cell_particles_ME1_ar.fill(0)
        self.bin_write_inds_ME2_ar.fill(0)
        self.cell_particles_ME2_ar.fill(0)
        self.bin_write_inds_ME3_ar.fill(0)
        self.cell_particles_ME3_ar.fill(0)
        self.bin_write_inds_HE_ar.fill(0)
        self.cell_particles_HE_ar.fill(0)
        self.bin_particles(self.big_tosum_ar_list[ng], self.d_cell_particles_HE_ar, self.d_cell_particles_ME1_ar, self.d_cell_particles_ME2_ar,
                           self.d_cell_particles_ME3_ar, self.d_cell_particles_LE_ar, self.d_bin_write_inds_HE_ar, self.d_bin_write_inds_ME1_ar, 
                           self.d_bin_write_inds_ME2_ar, self.d_bin_write_inds_ME3_ar, self.d_bin_write_inds_LE_ar, self.d_currxbins_list[ng], 
                           self.Nc_list[ng], self.N, self.max_ppc_realistic_HE, self.max_ppc_realistic_ME1, self.max_ppc_realistic_ME2,
                           self.max_ppc_realistic_ME3, self.max_ppc_realistic_LE, self.threads_per_block)
        ...
        # 3 mediums
        mmin_ids = cp.where(self.bin_write_inds_ME1_ar[:] > 0)[0]
        if (len(mmin_ids) > 0): 
            ME1min = cp.min(self.bin_write_inds_ME1_ar[cp.where(self.bin_write_inds_ME1_ar[:] > 0)[0]+0])
        else:
            ME1min = 100000
        ME1max = cp.max(self.bin_write_inds_ME1_ar)

        mmin_ids = cp.where(self.bin_write_inds_ME2_ar[:] > 0)[0]
        if (len(mmin_ids) > 0): 
            ME2min = cp.min(self.bin_write_inds_ME2_ar[cp.where(self.bin_write_inds_ME2_ar[:] > 0)[0]+0])
        else:
            ME2min = 100000
        ME2max = cp.max(self.bin_write_inds_ME2_ar)

        mmin_ids = cp.where(self.bin_write_inds_ME3_ar[:] > 0)[0]
        if (len(mmin_ids) > 0): 
            ME3min = cp.min(self.bin_write_inds_ME3_ar[cp.where(self.bin_write_inds_ME3_ar[:] > 0)[0]+0])
        else:
            ME3min = 100000
        ME3max = cp.max(self.bin_write_inds_ME3_ar)
        ...
        if (ME1min < 0.5*self.ttME1):
            self.need_to_resample = True
        if (ME2min < 0.5*self.ttME2):
            self.need_to_resample = True
        if (ME3min < 0.5*self.ttME3):
            self.need_to_resample = True
        if (HEmin < 0.5*self.ttHE):
            self.need_to_resample = True
        if (LEmax >= 3.0*self.ttLE):
            self.need_to_resample = True
        if (ME1max >= 3.0*self.ttME1):
            self.need_to_resample = True
        if (ME2max >= 3.0*self.ttME2):
            self.need_to_resample = True
        if (ME3max >= 3.0*self.ttME3):
            self.need_to_resample = True
