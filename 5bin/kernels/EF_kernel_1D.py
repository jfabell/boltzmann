...
def EF_kernel(
    ...
    V_rhs[0] = V_rhs[0] + (1.0 / (dx**2)) * V0 * math.sin(2 * cp.pi * freq * curr_t)
    ...
    for i in range(1, N-1):
        V_ar[N-2-i] = (1.0 / Vc_diag[N-2-i]) * (V_tempy[N-2-i] - Vc_lower_diag[N-2-i+1] * V_ar[N-2-i+1])

    ## ELECTRIC FIELD
    for i in range(N-2):
        E_ar[i+1] = -(V_ar[i+1] - V_ar[i]) / dx
    E_ar[0] = -(V_ar[0] - V0 * math.sin(2 * cp.pi * freq * curr_t)) / dx  # (V_1 - V_0)/dx
